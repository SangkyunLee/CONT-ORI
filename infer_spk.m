function infer_spk(dff_fac, bonlyrim,bspatial, ndct_time, exptypestr)
...
for ifinfo=1:length(finfo)
    strinx = strfind(finfo(ifinfo).Experiment,exptypestr);
    if length(finfo(ifinfo).Experiment)>=length(exptypestr)...
            && ~isempty(strinx) ...
            && strinx==1 ...
            && (strcmp(finfo(ifinfo).use,'1') || strcmp(finfo(ifinfo).use,'0') )
            ...
            if validcell
                ...
                for isubdat = 1 : Nseg
                    ...
                    if bonlyrim
                        ...
                        newpixinx_sub(:,isubdat) = find(mask(fill_list_sub{isubdat})==1);
                    end
                    if exist('navg','var') 
                        for iavg = 1 :navg
                            if iavg
                ...
                if bspatial
                    ...
                    [Nhat, ~, a, ~, history1]=getspikes4(double(dFF1),opts);
                    ...
                    dFF1 = (mF-F01)/F01;
                    opts.MAX_ITER=20;
                    [Nhat, ~, a, ~, history]=getspikes4(double(dFF1),opts);                 
                end
                atmp = zeros(size(F01));
                atmp(inxpix_posF0)=a;
                a1 = atmp;
