function get_subcellcrcon_subdata_decoding(iexp_type, DATA_thr_str,dpart, comcont, cellselopt, bshuffle,ctm,seslist,zmean)
...
if ~exist('bshuffle','var') || isempty(bshuffle)
    bshuffle = false;
end
...
currentFolder = pwd;
if bshuffle
    fnsave = sprintf('%s-SHUFFLE_%sSUBCELL-%s_CRSCON_SMLR_L2_ctm%0.2f.mat',dpart,fntmp{iexp_type}, dtype,ctm); %L2norm -regularization
else
    fnsave = sprintf('%s-%sSUBCELL-%s_CRSCON_SMLR_L2_ctm%0.2f.mat',dpart,fntmp{iexp_type},dtype,ctm); %L2norm -regularization
end
if bias
...
for ises = seslist 
    ...
    fndata1 = sprintf('%s_ctm%0.2fses%d-%s.mat',pprotype,ctm,ises,dpart);
