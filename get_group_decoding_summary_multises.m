...
if bpart
    fnsave = sprintf('SUMMARY_%s%.2f-%s_DEC_CELLGRP_ctm%0.2f.mat',fnpf,rThr,datainxstr,ctm);
else
...
% in clusters, which resulted in high decoding acc (rThr*maxdecod),
% participatig ratio of cells.
CELL_CBT=cell(length(sel_compset),length(ncellgrp_plot),length(contrasts),length(seslist));
% in clusters, which resulted in high decoding acc (rThr*maxdecod),
% participatig ratio of cells: info are set in cell indices.
...
bins =[0.0 :0.1: 1];
QuantInfo=zeros(3,2,length(sel_compset),length(contrasts),length(seslist));
P = zeros(length(bins)-1, 2, length(sel_compset), length(seslist));
