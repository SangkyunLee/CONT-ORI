...
%  A= D0(1).meanimg(201:302,40:142);
  A= D0(1).meanimg((211:280)+10,(51:120)-23);
%  A(A(:)>3300)=3300;
 thr = 3000 
 A(A(:)>thr)=thr;
 A = (A-min(A(:)))/(max(A(:))-min(A(:)));
%  A =histeq(A);
 figure('Position',[100 100 size(A,1)*1.5 size(A,2)*1.5]); imagesc(A); colormap('gray'); axis image;
 axis off
...
for i = 1:nC
    ...
    M = Md1.mresp(:,1,icell);
    maxM = max(max(Md1.mresp(:,1:2,icell)));
    a = 1/maxM;
    Mn = M*a;
    S = Md1.semresp(:,1,icell);
    Sn = S*a;
    errorbar([0:30:330],Mn,Sn,'.-','Color',clr{1},'linewidth',1.8,'markersize',15);
    M = Md1.mresp(:,2,icell);    
    Mn = M*a;
    S = Md1.semresp(:,2,icell);
    Sn = S*a;
    errorbar([0:30:330],Mn,Sn,'.-','Color',clr{2},'linewidth',1.8,'markersize',15);
...
%         xlim([-15 110])
    ...
    xlim([-5 335])
    ylim([0 1.5]);
   % set(gca,'linewidth',1.5,'xtick',[0 180], 'xticklabel',[],'ytick',[0.5 1], 'yticklabel',[]);
     axis off
    kk = kk + 1;
end
% axis on
...
figure('Position',[680 173 400*1.2 300*1.2]); hold on;
for i = 1: length(cid)
    ic = cellinx(CLIST(cid(i)));
    CLIST(cid(i)) 
    ...
    a = D0(iscan).Nhat(inxt,ic);
    a = a/m0*s*0.7 + b0; 
    plot(t,dff,'k','linewidth',1.5);
    plot(t,a,'Color',[0.5 0.5 0.5],'linewidth',1.5);
    b0 =  max(dff)+1;
end
xlim([200 400])
axis off
...
figure('position',[680   558   400*1.2   300*1.2]); hold on;
...
set(gca,'FontSize',22,'XTick',[-0.5:0.5:2]);
xlim([-0.55 2.05])
ylim([2 8])
...
figure('position',[680   378   327   420]);
for i0=1:2
   i = cellist(i0); 
subplot(2,1,i0);
hold on;
...
plot(0:30:330,nr100,'r.-','LineWidth',2,'markersize',15);
plot(0:30:330,nr40,'b.-','LineWidth',2,'markersize',15);
plot(0:30:330,(a*r100+b)/n,'k.-','LineWidth',2,'markersize',15);
