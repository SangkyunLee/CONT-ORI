            ...
            for ithr = 1 : length(CELLSEL_THRs)
                ...
                for iter = 1 : Niter
                    ...
                    if length(out_ch) == opts.Nch
                        DEC_SELCELL{icomp0,ises}(ithr,iter,icont)=Inf;
                    else
                        [out ] = par_cv_classification(data,evtsel,opts);
                        DEC_SELCELL{icomp0,ises}(ithr,iter,icont) = mean(out.acc_test);
                        if isempty(out_ch)
                            DEC_SELCELL{icomp0,ises}(ithr,iter+1:Niter,icont)= DEC_SELCELL{icomp0,ises}(ithr,1,icont); 
                            continue;
                        end
                    end
                end
