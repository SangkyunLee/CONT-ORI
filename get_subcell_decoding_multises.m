...
% SEL_METHOD='weightl1'
CELLSEL_THRs=[0 1 3 5 10]%0:5:30;
SEL_METHOD='ncell';
...
currentFolder = pwd;
modelfname = sprintf('%s-%sSMLR_L1_ctm%0.2f.mat',Mfnpf,fntmp{iexp_type},ctm); %L1norm -regularization
fullmodelfname = fullfile(fileparts(currentFolder),'NEW_DECODING',exp_type{iexp_type},DATA_thr_str, modelfname);

% 
fnsave = sprintf('%s_%s-%sCELLSEL_%s_SMLR_L2_ctm%0.2f.mat',Mfnpf,Pfnpf,fntmp{iexp_type},upper(SEL_METHOD),ctm); %L2norm -regularization
...
% fnsave = sprintf('%s_%s-%sCELLSEL_%s_SMLR_ctm%0.2f.mat',Mfnpf,Pfnpf,fntmp{iexp_type},upper(SEL_METHOD),ctm); %L2norm -regularization
% fullfnsav = fullfile(fileparts(currentFolder),'NEW_DECODING',exp_type{iexp_type},DATA_thr_str, fnsave);
% lambda1s=0;
% lambda2s=0;
...
for ises = seslist 
    ...
    fndata = sprintf('%s_ctm%0.2fses%d-%s.mat',pprotype,ctm,ises,Pfnpf);
    ...
    % get common celllist and data between sessions
    Mcellinx = sort(M4c.CELLINX4DECODING(:,1,1));
    imap = zeros(1,max(cellinx_sel));
    imap(cellinx_sel)=1:length(cellinx_sel);
    cellinx_sel1 = intersect(cellinx_sel,Mcellinx(:)');
